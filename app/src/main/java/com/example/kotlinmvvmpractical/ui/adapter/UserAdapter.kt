...
class UserAdapter(var context: Context) : PagedListAdapter<UsersItem, UserAdapter.PhotoViewHolder?>(UserResponse.CALLBACK) {
    ...
    override fun onBindViewHolder(
        ...
        if(getItem(i)?.items?.size!! % 2==0)
            photoViewHolder.rvUserImagesList.layoutManager=GridLayoutManager(context,4)
        else
            photoViewHolder.rvUserImagesList.layoutManager=GridLayoutManager(context,5)
